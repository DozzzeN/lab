...
from alignment import alignFloat, genAlign, genLongestContinuous
...
for staInd in range(0, 10 * intvl + 1, intvl):
    ...
    for i in range(opNums):
        ...
        allOpsIndexA.append(sortCSIi1[index[i]])
        # if random.randint(0, 1) == 0:
        #     allOpsIndexA.append(sortCSIa1[index[i]])
        # else:
        #     allOpsIndexA.append(random.uniform(min(sortCSIa1), max(sortCSIa1)))
    ...
    # plt.figure()
    # plt.plot(range(len(sortCSIa1P)), sortCSIa1P, color="red", linewidth=.5, label="aP")
    # plt.legend(loc='upper left')
    # plt.show()
    #
    # plt.figure()
    # plt.plot(range(len(sortCSIa1)), sortCSIa1, color="blue", linewidth=.5, label="a")
    # plt.legend(loc='upper left')
    # plt.show()
