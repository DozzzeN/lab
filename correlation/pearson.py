...
for staInd in range(0, 10 * intvl + 1, intvl):
    ...
    # episode中的点求相关系数
    def CSIEpisode(CSITmp, step):
        n = int(len(CSITmp) / step)
        episode = [[0 for _ in range(n)] for _ in range(n)]

        row = 0
        col = 0
        for i in range(0, len(CSITmp), step):
            for j in range(0, len(CSITmp), step):
                # tmp1 = CSITmp[i:i + step]
                # tmp2 = CSITmp[j:j + step]
                episode[row][col] = pearsonr(CSITmp[i:i + step], CSITmp[j:j + step])[0]
                col = col + 1
            row = row + 1
            col = 0
        return episode
    ...
    episodeCSIa1 = CSIEpisode(sortInda, 4)
    episodeCSIb1 = CSIEpisode(sortIndb, 4)
    episodeCSIe1 = CSIEpisode(sortInde, 4)
    episodeCSIn1 = CSIEpisode(sortIndn, 4)
    ...
    djA1 = dj.DijkstraSPF(a1Graph, 7)
    djB1 = dj.DijkstraSPF(b1Graph, 7)
    djE1 = dj.DijkstraSPF(e1Graph, 7)
    djN1 = dj.DijkstraSPF(n1Graph, 7)
    ...
    sum1 = len(pathA1)
    sum2 = 0
    for sum1index in range(0, sum1):
        sum2 += (len(pathA1[sum1index]) - len(pathB1[sum1index]) == 0)
    sum3 = 0
    for sum2index in range(0, sum1):
        sum3 += (len(pathA1[sum2index]) - len(pathE1[sum2index]) == 0)
    sum4 = 0
    for sum3index in range(0, sum1):
        sum4 += (len(pathA1[sum3index]) - len(pathN1[sum3index]) == 0)
